...
const AddUpDataComponent: React.FC<ModalFormProps> = props => {
    ...
    row_6: [
      {
        ...
        placeholder: '请选择',
        onChange: item => {
          setVisiable(item.target.value);
        },
        validator: validator.onlyRequier,
        defulatVal: 'merchantAgent',
        formData: common.merchantAccountType.map(item => {
          return { name: item.dictItemName, type: item.dictItemCode };
        }),
      },
      {
        type: 'select',
        ...
        placeholder: ' ',
        validator: validator.onlyRequier,
        defulatVal: defulat.inviterId,
        formData: [
          { name: '没有上级', type: ' ' },
          ...allMerchantAgent.map(item => {
            return { name: item.merchantName, type: item.id };
          }),
        ],
      },
    ],
  ...
  const generateSecretKey = () => {
    dispatch({
      type: 'merchant/generateSecretKey',
      payload: {},
    }).then(data => {
    ...
    >
      {helpers.isJudge(hasModity)(
        ...
        <Form>
          ...
          <Row gutter={[8, 8]}>
            {// {generateCols(addRenderItems.row_6, 12)}
            helpers.isJudge(visiable === 'merchantAgent')(
              <Col key={`row99`} span={12}>
                <GenerateFormCompoents formItems={[addRenderItems.row_6[0]]} form={form} />
              </Col>,
              <Row gutter={[8, 8]}>{generateCols(addRenderItems.row_6, 12)}</Row>,
            )}
          </Row>
          <Row gutter={[8, 8]}>
            <Col key={`row99`} span={24}>
