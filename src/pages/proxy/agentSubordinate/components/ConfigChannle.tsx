...
const ConfigChannle: React.FC<ModalFormProps> = props => {
  ...
  const { dispatch, handleModalVisible, actionRef } = props;
  ...
  const getDatas = () => {
    ...
    return dispatch({
      type: 'agentSubordinate/findLowerLevelGatheringChannelRate',
      payload: { params },
    }).then(data => {
  ...
  const okHandle = () => {
    const params = { channelRates: fromData, merchantId: defulat.id };
    dispatch({
      type: 'agentSubordinate/saveGatheringChannelRate',
      payload: { params },
    }).then(data => {
