...
const Agent = props => {
  ...
  const insertModityRole = params => {
    ...
    const type = helpers.isJudge(hasModity)('creditMember/update', 'creditMember/add');
    dispatch({
      type,
      payload: { params },
    }).then(data => {
  ...
  const updateMoneyPwd = params => {
    ...
    dispatch({
      type: 'creditMember/modifyMoneyPwd',
  ...
  const syncReceiveOrderChanne = params => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        return dispatch({
          type: 'creditMember/syncReceiveOrderChanne',
          payload: { params: { userAccountId } },
        }).then(data => {
          message.success(constant.alertMessage.UPDATE_SUCCESS);
          actionRef.current?.reload();
